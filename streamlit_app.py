...
class VideoStreamHandler:
    ...
    def compare_faces_directly(face_encoding, face_data_dict):
        ...
        threshold = 0.6  # Lower is stricter, higher is more lenient
    ...
    def process_video_stream(self):
        """
        Function to process the RTSP video stream.
        """
        ...
        while not self.stop_event.is_set():
            ...
            if frame_count % check_interval == 0:
                with self.face_data_lock:
                    self.face_data = self.load_face_ids_from_db_direct()
            ...
            with self.face_data_lock:
                current_face_data = self.face_data.copy()
            ...
            for (top, right, bottom, left) in face_locations:
                ...
                name, status = self.compare_faces_directly(encoding, current_face_data)
...
def main():
    ...
    if menu == "Video Stream":
        ...
        if uploaded_file is not None:
            ...
            if st.button("Predict"):
                try:
                    ...
                    # Load face data
                    face_data = video_stream_handler.load_face_ids_from_db_direct()

                    name, status = video_stream_handler.compare_faces_directly(encoding, face_data)
        ...
        if st.button("Reload Face Data"):
            video_stream_handler.face_data = video_stream_handler.load_face_ids_from_db_direct()
